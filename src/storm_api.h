...
BOOL STORMAPI SNetCreateGame( char *pszGameName, char *pszGamePassword, char *pszGameStatString, DWORD dwGameType, char *GameTemplateData, int GameTemplateSize, int playerCount, char *creatorName, char *a9, int *playerID);
BOOL STORMAPI SNetDestroy();
// SNetEnumProviders @ 103
BOOL STORMAPI SNetEnumProviders(int (__stdcall *callback)(DWORD, DWORD, DWORD/*, DWORD*/) /*, int mincaps*/); // нет тут второго параметра и четвертого в указателе на функцию ни в одной версии storm.dll

BOOL STORMAPI SNetEnumGames(int (__stdcall *callback)(DWORD, DWORD, DWORD), int *hintnextcall);
...
BOOL
STORMAPI
SNetDropPlayer(int playerid,DWORD flags);
...
typedef struct _CAPS
{
  ...
  DWORD dwTimeout;             // some kind of timeout or timer related
 ...
 *  Returns TRUE if the function was called successfully and FALSE otherwise.
 */
BOOL STORMAPI SNetInitializeProvider( DWORD providerName, client_info2 *gameClientInfo, user_info2 *userData, battle_info2 *bnCallbacks, module_info *moduleData);
...
BOOL
STORMAPI
SNetSendMessage( int playerID, void *data, size_t databytes );
...
BOOL
STORMAPI
SNetSendTurn( char *data, size_t databytes );
...
BOOL STORMAPI SDlgBltToWindow(HWND hWnd, HRGN a2, char *a3, int a4, void *buffer, RECT *rct, SIZE *size, int a8, int a9, DWORD rop); // was SDlgBltToWindowE
BOOL STORMAPI SDlgSetBitmap(HWND hWnd, int a2, char *src, int mask1, int flags, int a6, int a7, int width, int a9, int mask2); // SDlgSetBitmapE
 ...
 */
BOOL STORMAPI SBmpDecodeImage( DWORD dwImgType, void *pSrcBuffer, DWORD dwSrcBuffersize, PALETTEENTRY *pPalette = NULL, void *pDstBuffer = NULL, DWORD dwDstBuffersize = 0, DWORD *pdwWidth = NULL, DWORD *pdwHeight = NULL, DWORD *pdwBpp = NULL); 
 ...
 */
BOOL STORMAPI SBmpLoadImage( char *pszFileName, PALETTEENTRY *pPalette = NULL, void *pBuffer = NULL, DWORD dwBuffersize = 0, DWORD *pdwWidth = NULL, DWORD *pdwHeight = NULL, DWORD *pdwBpp = NULL);
 ...
 */
BOOL STORMAPI SBmpSaveImage( char *pszFileName, PALETTEENTRY *pPalette, void *pBuffer, DWORD dwWidth, DWORD dwHeight, DWORD dwBpp = 8);
...
void*
STORMAPI
SMemAlloc( size_t amount, char  *logfilename, int   logline, char  defaultValue = 0);
...
BOOL STORMAPI STransCreate(void *pBuffer, int width, int height, int bpp, int* a5, int bufferSize, HANDLE *phTransOut); // was STransCreateE
