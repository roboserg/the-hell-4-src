...
BOOL STORMAPI SNetInitializeProvider(DWORD providerName, client_info2 *gameClientInfo, user_info2 *userData, battle_info2 *bnCallbacks, module_info *moduleData) rBool;
...
HGDIOBJ STORMAPI SDlgDefDialogProc(HWND hDlg, int DlgType, HDC textLabel, HWND hWnd) rPVoid;
...
BOOL STORMAPI SFileDdaBeginEx(HANDLE directsound, DWORD flags, DWORD mask, unsigned __int32 lDistanceToMove, signed __int32 volume, int a6, int a7) rBool;
...
BOOL STORMAPI SFileDdaSetVolume(HANDLE directsound, int bigvolume, int volume) rBool;
...
BOOL STORMAPI SGdiExtTextOut(int a1, int a2, int a3, int a4, unsigned int a8, int a6, int a7, const char *string, unsigned int arg20) rBool;
...
BOOL STORMAPI STransCreate(void *pBuffer, int width, int height, int bpp, int* a5, int bufferSize, HANDLE *phTransOut) rBool;
