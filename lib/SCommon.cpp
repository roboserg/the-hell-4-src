...
static int  bStormBufferCreated = FALSE;
...
// uint value in block position. And if we know encrypted and decrypted value,
// we can find the decryption key !!!
//
// hf    - MPQ file handle
// block - uint array of block positions
// ch    - Decrypted value of the first block pos

uint DetectFileSeed(uint * block, uint decrypted)
{
    uint saveSeed1;
    uint temp = *block ^ decrypted;    // temp = seed1 + seed2
    ...
    for(int i = 0; i < 0x100; i++)      // Try all 255 possibilities
    {
        ...
        // Try the first uint (We exactly know the value)
...
uint DetectFileSeed2(uint * pdwBlock, UINT nDwords, ...)
{
    va_list argList;
    uint dwDecrypted[0x10];
    uint saveSeed1;
    uint dwTemp;
    uint i, j;
    ...
    for(i = 0; i < nDwords; i++)
        dwDecrypted[i] = va_arg(argList, uint);
    va_end(argList);
...
void EncryptMPQBlock(uint * block, uint dwLength, uint dwSeed1)
{
    uint dwSeed2 = 0xEEEEEEEE;
    uint ch;
...
void DecryptMPQBlock(uint * block, uint dwLength, uint dwSeed1)
{
    uint dwSeed2 = 0xEEEEEEEE;
    uint ch;
...
{
    BYTE * pbKey   = (BYTE *)szFileName;
    uint  dwSeed1 = 0x7FED7FED;          // EBX
    uint  dwSeed2 = 0xEEEEEEEE;          // ESI
    uint  ch;
...
TMPQHash * GetHashEntry(TMPQArchive * ha,  char * szFileName)
{
    ...
    uint dwIndex = (uint)(DWORD_PTR)szFileName;
    uint dwName1;
    uint dwName2;
...
TMPQHash * FindFreeHashEntry(TMPQArchive * ha,  char * szFileName)
{
    ...
    uint dwIndex = DecryptHashIndex(ha, szFileName);
    uint dwName1 = DecryptName1(szFileName);
    uint dwName2 = DecryptName2(szFileName);
    uint dwBlockIndex = 0xFFFFFFFF;
    ...
    int * pbReplaced)
{
    ...
    uint dwBlockPosLen = 0;            // Length of the file block offset (in bytes)
    uint dwTransferred = 0;            // Number of bytes read or written
    uint dwFileSizeHigh = 0;           // High 32 bits of the file size
    uint dwFileSize = 0;               // Low 32-bits of the file size
    int bReplaced = FALSE;             // TRUE if replaced, FALSE if added
    ...
    {
        dwFileSize = GetFileSize(hFile, (LPDWORD)&dwFileSizeHigh);
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        // Set the hash index
        hf->dwHashIndex = (uint)(hf->pHash - ha->pHashTable);
    }
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        hf->dwBlockIndex = (uint)(hf->pBlock - ha->pBlockTable);
    ...
    {
        hf->pdwBlockPos = ALLOCMEM(uint, hf->nBlocks + 1);
    ...
    if(nError == ERROR_SUCCESS && (dwFlags & MPQ_FILE_COMPRESSED))
    {
        dwBlockPosLen = hf->nBlocks * sizeof(uint);
        if(dwFlags & MPQ_FILE_HAS_EXTRA)
            dwBlockPosLen += sizeof(uint);
        ...
        WriteFile(ha->hFile, hf->pdwBlockPos, dwBlockPosLen, (LPDWORD)&dwTransferred, NULL);
        BSWAP_ARRAY32_UNSIGNED(hf->pdwBlockPos, 1);
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        for(nBlock = 0; nBlock < hf->nBlocks-1; nBlock++)
        {
            ...
            ReadFile(hFile, hf->pbFileBuffer, ha->dwBlockSize, (LPDWORD)&dwInLength, NULL);
            if(dwInLength == 0)
                break;
            ...
            {
                BSWAP_ARRAY32_UNSIGNED((uint *)pbToWrite, dwOutLength / sizeof(uint));
                EncryptMPQBlock((uint *)pbToWrite, dwOutLength, hf->dwSeed1 + nBlock);
                BSWAP_ARRAY32_UNSIGNED((uint *)pbToWrite, dwOutLength / sizeof(uint));
            }
...
int SaveMPQTables(TMPQArchive * ha)
{
    ...
    uint dwBytes;
    uint dwWritten;
    uint dwBuffSize = STORMLIB_MAX(ha->pHeader->dwHashTableSize, ha->pHeader->dwBlockTableSize);
    int   nError = ERROR_SUCCESS;
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        WriteFile(ha->hFile, ha->pHeader, dwHeaderSize, (LPDWORD)&dwWritten, NULL);
        BSWAP_TMPQHEADER(ha->pHeader);
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        // Convert to little endian for file save
        EncryptHashTable((uint *)pbBuffer, (BYTE *)"(hash table)", dwBytes >> 2);
        BSWAP_ARRAY32_UNSIGNED((uint *)pbBuffer, dwBytes / sizeof(uint));
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        EncryptBlockTable((uint *)pbBuffer, (BYTE *)"(block table)", dwBytes >> 2);
