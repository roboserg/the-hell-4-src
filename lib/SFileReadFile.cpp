...
static uint __stdcall ReadMPQBlocks(TMPQFile * hf, uint dwBlockPos, BYTE * buffer, uint blockBytes)
{
    ...
    uint   dwToRead;                   // Number of bytes to read
    uint   blockNum;                   // Block number (needed for decrypt)
    uint   dwBytesRead = 0;            // Total number of bytes read
    ...
    if((hf->pBlock->dwFlags & MPQ_FILE_COMPRESSED) && hf->bBlockPosLoaded == FALSE)
    {
        ...
        dwToRead = (hf->nBlocks+1) * sizeof(uint);
        ...
        ReadFile(ha->hFile, hf->pdwBlockPos, dwToRead, (LPDWORD)&dwBytesRead, NULL);
    ...
    ReadFile(ha->hFile, tempBuffer, dwToRead, (LPDWORD)&dwBytesRead, NULL);

    // Block processing part.
    uint blockStart = 0;               // Index of block start in work buffer
    uint blockSize  = STORMLIB_MIN(blockBytes, ha->dwBlockSize);
    ...
    for(i = 0; i < nBlocks; i++, index++)
    {
        ...
        if(hf->pBlock->dwFlags & MPQ_FILE_ENCRYPTED)
        {
            BSWAP_ARRAY32_UNSIGNED((uint *)inputBuffer, blockSize / sizeof(uint));
...
static uint __stdcall ReadMPQFileSingleUnit(TMPQFile * hf, uint dwFilePos, BYTE * pbBuffer, uint dwToRead)
{
    ...
    if(hf->pBlock->dwCSize < hf->pBlock->dwFSize)
    {
        if(hf->pbFileBuffer == NULL)
        {
            ...
            if(inputBuffer != NULL)
            {
                ...
                ReadFile(ha->hFile, inputBuffer, inputBufferSize, (LPDWORD)&dwBytesRead, NULL);
    ...
    else
    {
        ...
        ReadFile(ha->hFile, pbBuffer, dwToRead, (LPDWORD)&dwBytesRead, NULL);
    }
...
static uint __stdcall ReadMPQFile(TMPQFile * hf, uint dwFilePos, BYTE * pbBuffer, uint dwToRead)
{
    TMPQArchive * ha = hf->ha; 
...
int __stdcall SFileReadFile_lib(HANDLE hFile, VOID * lpBuffer, uint dwToRead, uint * pdwRead, LPOVERLAPPED lpOverlapped)
{
    ...
    if(nError == ERROR_SUCCESS && hf->hFile != INVALID_HANDLE_VALUE)
    {
        ...
        ReadFile(hf->hFile, lpBuffer, dwToRead, (LPDWORD)&dwTransferred, lpOverlapped);
...
uint __stdcall SFileGetFilePos(HANDLE hFile, uint * pdwFilePosHigh)
{
    TMPQFile * hf = (TMPQFile *)hFile;
...
uint __stdcall SFileGetFileSize_lib(HANDLE hFile, uint * pdwFileSizeHigh)
{
    TMPQFile * hf = (TMPQFile *)hFile;
    ...
    // If opened as plain file, ...
    if(hf->hFile != INVALID_HANDLE_VALUE)
        return GetFileSize(hf->hFile, (LPDWORD)pdwFileSizeHigh);
...
int __stdcall SFileGetFileName(HANDLE hFile, char * szFileName)
{
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        dwFilePos = SFileSetFilePointer_lib(hf, 0, NULL, FILE_CURRENT);   
        SFileReadFile_lib(hFile, &dwFirstBytes, sizeof(dwFirstBytes), NULL);
        BSWAP_ARRAY32_UNSIGNED(dwFirstBytes, sizeof(dwFirstBytes) / sizeof(uint));
        SFileSetFilePointer_lib(hf, dwFilePos, NULL, FILE_BEGIN);
    }
