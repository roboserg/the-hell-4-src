...
int SAttrFileLoad(TMPQArchive * ha)
{
    ...
    if(!SFileOpenFileEx_lib((HANDLE)ha, ATTRIBUTES_NAME, 0, &hFile))
        nError = GetLastError();
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        dwToRead = sizeof(uint) + sizeof(uint);
        SFileReadFile_lib(hFile, pAttr, dwToRead, &dwBytesRead, NULL);
    ...
    if(nError == ERROR_SUCCESS && (pAttr->dwFlags & MPQ_ATTRIBUTE_CRC32))
    {
        ...
        if(pAttr->pCrc32 != NULL)
        {
            ...
            SFileReadFile_lib(hFile, pAttr->pCrc32, dwToRead, &dwBytesRead, NULL);
            if(dwBytesRead != dwToRead)
                nError = ERROR_FILE_CORRUPT;
        }
        else
    ...
    if(nError == ERROR_SUCCESS && (pAttr->dwFlags & MPQ_ATTRIBUTE_FILETIME))
    {
        ...
        if(pAttr->pFileTime != NULL)
        {
            ...
            SFileReadFile_lib(hFile, pAttr->pFileTime, dwToRead, &dwBytesRead, NULL);
            if(dwBytesRead != dwToRead)
                nError = ERROR_FILE_CORRUPT;
        }
        else
    ...
    if(nError == ERROR_SUCCESS && (pAttr->dwFlags & MPQ_ATTRIBUTE_MD5))
    {
        ...
        if(pAttr->pMd5 != NULL)
        {
            ...
            SFileReadFile_lib(hFile, pAttr->pMd5, dwToRead, &dwBytesRead, NULL);
            if(dwBytesRead != dwToRead)
                nError = ERROR_FILE_CORRUPT;
        }
        else
...
int SAttrFileSaveToMpq(TMPQArchive * ha)
{
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        dwToWrite = sizeof(uint) + sizeof(uint);
        WriteFile(hFile, ha->pAttributes, dwToWrite, (LPDWORD)&dwWritten, NULL);
    ...
    if(nError == ERROR_SUCCESS && ha->pAttributes->pCrc32 != NULL)
    {
        ...
        WriteFile(hFile, ha->pAttributes->pCrc32, dwToWrite, (LPDWORD)&dwWritten, NULL);
        if(dwWritten != dwToWrite)
            nError = ERROR_DISK_FULL;
    }
    ...
    if(nError == ERROR_SUCCESS && ha->pAttributes->pFileTime != NULL)
    {
        ...
        WriteFile(hFile, ha->pAttributes->pFileTime, dwToWrite, (LPDWORD)&dwWritten, NULL);
        if(dwWritten != dwToWrite)
            nError = ERROR_DISK_FULL;
    }
    ...
    if(nError == ERROR_SUCCESS && ha->pAttributes->pMd5 != NULL)
    {
        ...
        WriteFile(hFile, ha->pAttributes->pMd5, dwToWrite, (LPDWORD)&dwWritten, NULL);
        if(dwWritten != dwToWrite)
            nError = ERROR_DISK_FULL;
    }
...
int __stdcall SFileVerifyFile(HANDLE hMpq,  char * szFileName, uint dwFlags)
{
    ...
    if(SFileOpenFileEx_lib(hMpq, szFileName, 0, &hFile))
    {
        ...
        {
            // Read data from file
            SFileReadFile_lib(hFile, Buffer, sizeof(Buffer), &dwBytesRead, NULL);
            if(dwBytesRead == 0)
                break;
