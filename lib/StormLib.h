...
struct TMPQCRC32
{
    uint dwValue;                      // Value of CRC32 for each block
};
...
struct TMPQFileTime
{
    uint dwFileTimeLow;                // Low uint of the FILETIME
    uint dwFileTimeHigh;               // High uint of the FILETIME
};
...
struct TMPQAttr
{
    uint dwVersion;                    // Version of the (attributes) file. Must be 100 (0x64)
    uint dwFlags;                      // See MPQ_ATTRIBUTE_XXXX
...
// Archive handle structure / 432 bytes
struct TMPQArchive
{
// 	TMPQArchive * pNext;				// +0	// Next archive (used by Storm.dll only)
// 	TMPQArchive * pPrev;				// +4	// Previous archive (used by Storm.dll only)
    char          szFileName[260];		// +8	// Opened archive file name [MAX_PATH]
    HANDLE        hFile;				// +268	// File handle
    uint		  dwPriority;			// +272	// Priority of the archive
    LARGE_INTEGER ShuntPos;             // +276	// MPQShunt offset (only valid if a shunt is present)
    LARGE_INTEGER MpqPos;               // +284	// File header offset (relative to the begin of the file)
    LARGE_INTEGER HashTablePos;         // +292	// Hash table offset (relative to the begin of the file)
    LARGE_INTEGER BlockTablePos;        // +300	// Block table offset (relative to the begin of the file)
    LARGE_INTEGER ExtBlockTablePos;     // +308	// Ext. block table offset (relative to the begin of the file)
    LARGE_INTEGER MpqSize;              // +316	// Size of MPQ archive

	TMPQFile    * pLastFile;            // +324 // Recently read file
    uint		  dwBlockPos;			// +328 // Position of loaded block in the file
    uint          dwBlockSize;			// +332 // Size of file block
    BYTE        * pbBlockBuffer;        // +336	// Buffer (cache) for file block
    uint          dwBuffPos;			// +340	// Position in block buffer
    TMPQShunt   * pShunt;               // +344	// MPQ shunt (NULL if not present in the file)
    TMPQHeader2 * pHeader;              // +348 // MPQ file header
    TMPQHash    * pHashTable;           // +352 // Hash table
    TMPQBlock   * pBlockTable;          // +356 // Block table
    TMPQBlockEx * pExtBlockTable;       // +360 // Extended block table
    
    TMPQShunt     Shunt;                // +364 // MPQ shunt. Valid only when ID_MPQ_SHUNT has been found
    TMPQHeader2   Header;               // +376 // MPQ header

    TMPQAttr    * pAttributes;          // +420 // MPQ attributes from "(attributes)" file (NULL if none)
    TFileNode  ** pListFile;            // +424 // File name array
    uint          dwFlags;				// +428 // See MPQ_FLAG_XXXXX
};
...
struct TMPQFile
{
    ...
    uint          nBlocks;             // Number of blocks in the file (incl. the last incomplete one)
    int           bBlockPosLoaded;     // TRUE if block positions loaded
    ...
    uint          dwBlockIndex;        // Index to Block table
    char           szFileName[1];       // File name (variable length)
};
...
struct TMPQSearch
{
    ...
    uint  dwNextIndex;                 // Next hash index to be checked
...
typedef LCID  (__stdcall * SFILESETLOCALE)(LCID);
typedef int  (__stdcall * SFILEOPENARCHIVE)( char *, uint, uint, HANDLE *);
typedef int  (__stdcall * SFILECLOSEARCHIVE)(HANDLE);
typedef int  (__stdcall * SFILEOPENFILEEX)(HANDLE,  char *, uint, HANDLE *);
typedef int  (__stdcall * SFILECLOSEFILE)(HANDLE);
typedef uint (__stdcall * SFILEGETFILESIZE)(HANDLE, uint *);
typedef uint (__stdcall * SFILESETFILEPOINTER)(HANDLE, LONG, LONG *, uint);
typedef int  (__stdcall * SFILEREADFILE)(HANDLE, VOID *, uint, uint *, LPOVERLAPPED);
...
int  __stdcall SFileCloseArchive(HANDLE hMpq);

// File opening/closing
int  __stdcall SFileOpenFileEx_lib(HANDLE hMpq,  char * szFileName, uint dwSearchScope, HANDLE * phFile);
int  __stdcall SFileCloseFile_lib(HANDLE hFile);

// File I/O
uint __stdcall SFileGetFilePos(HANDLE hFile, uint * pdwFilePosHigh = NULL);
uint __stdcall SFileGetFileSize_lib(HANDLE hFile, uint * pdwFileSizeHigh = NULL);
uint __stdcall SFileSetFilePointer_lib(HANDLE hFile, LONG lFilePos, LONG * pdwFilePosHigh, uint dwMethod);
int  __stdcall SFileReadFile_lib(HANDLE hFile, VOID * lpBuffer, uint dwToRead, uint * pdwRead = NULL, LPOVERLAPPED lpOverlapped = NULL);

// Adds another listfile into MPQ. The currently added listfile(s) remain,
// so you can use this API to combining more listfiles.
// Note that this function is internally called by SFileFindFirstFile
int   __stdcall SFileAddListFile(HANDLE hMpq,  char * szListFile);

//-----------------------------------------------------------------------------
// Functions in StormLib - not implemented in Storm.dll

// Archive creating and editing
int  __stdcall SFileCreateArchiveEx( char * szMpqName, uint dwCreationDisposition, uint dwHashTableSize, HANDLE * phMpq);
int  __stdcall SFileAddFile(HANDLE hMpq,  char * szFileName,  char * szArchivedName, uint dwFlags); 
int  __stdcall SFileAddWave(HANDLE hMpq,  char * szFileName,  char * szArchivedName, uint dwFlags, uint dwQuality); 
int  __stdcall SFileRemoveFile(HANDLE hMpq,  char * szFileName, uint dwSearchScope = SFILE_OPEN_BY_INDEX);
int  __stdcall SFileRenameFile(HANDLE hMpq,  char * szOldFileName,  char * szNewFileName);
int  __stdcall SFileSetFileLocale(HANDLE hFile, LCID lcNewLocale);

// Retrieving info about the file
int  __stdcall SFileHasFile(HANDLE hMpq, char * szFileName);
int  __stdcall SFileGetFileName(HANDLE hFile, char * szFileName);
DWORD_PTR __stdcall SFileGetFileInfo(HANDLE hMpqOrFile, uint dwInfoType);

// File search
// Note that the SFileFindFirstFileEx has been removed. Use SListFileFindFirst/Next
HANDLE __stdcall SFileFindFirstFile(HANDLE hMpq,  char * szMask, SFILE_FIND_DATA * lpFindFileData,  char * szListFile);
int   __stdcall SFileFindNextFile(HANDLE hFind, SFILE_FIND_DATA * lpFindFileData);
int   __stdcall SFileFindClose(HANDLE hFind);

// Listfile search
HANDLE SListFileFindFirstFile(HANDLE hMpq,  char * szListFile,  char * szMask, SFILE_FIND_DATA * lpFindFileData);
int   SListFileFindNextFile(HANDLE hFind, SFILE_FIND_DATA * lpFindFileData);
int   SListFileFindClose(HANDLE hFind);

// Archive compacting
typedef void  (__stdcall * COMPACTCB)(void * lpUserData, uint dwWorkType, uint dwParam1, uint dwParam2);
int  __stdcall SFileSetCompactCallback(HANDLE hMpq, COMPACTCB CompactCB, void * lpData);
int  __stdcall SFileCompactArchive(HANDLE hMpq,  char * szListFile = NULL, int bReserved = 0);

// Locale support
int   __stdcall SFileEnumLocales(HANDLE hMpq,  char * szFileName, LCID * plcLocales, uint * pdwMaxLocales, uint dwSearchScope);

// (De)compression
int __stdcall SCompCompress   (char * pbOutBuffer, int * pdwOutLength, char * pbInBuffer, int dwInLength, int uCompressions, int nCmpType, int nCmpLevel);
int __stdcall SCompDecompress (char * pbOutBuffer, int * pdwOutLength, char * pbInBuffer, int dwInLength);
...
int __stdcall SCompSetDataCompression(int nDataCompression);

// Verifies file against its extended attributes (depending on dwFlags).
// For dwFlags, use one or more of MPQ_ATTRIBUTE_MD5
int  __stdcall SFileVerifyFile(HANDLE hMpq,  char * szFileName, uint dwFlags);
...
USHORT  __stdcall celGetFrameCount(BYTE * fileBuf);
BYTE  * __stdcall celGetFrameData(BYTE *fileBuf, BYTE *palette, USHORT xsize, USHORT frame, USHORT *ysize, USHORT *maxX=NULL);
USHORT  __stdcall cl2GetFrameCount(BYTE *fileBuf);
BYTE ** __stdcall cl2GetDirData(BYTE *fileBuf, BYTE *palette, USHORT xsize, USHORT dir, USHORT *ysize);
BYTE  * __stdcall pcxGetData(BYTE *filebuf, uint filesize, BYTE transcol, USHORT *xsize, USHORT *ysize);
