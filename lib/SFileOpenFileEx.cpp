...
static int OpenLocalFile( char * szFileName, HANDLE * phFile)
{
    TMPQFile * hf = NULL;
...
int __stdcall SFileEnumLocales(
    HANDLE hMPQ,
     char * szFileName,
    LCID * plcLocales,
    uint * pdwMaxLocales,
    uint dwSearchScope)
{
    ...
    if(nError == ERROR_SUCCESS)
    {
        uint dwMaxLocales = *pdwMaxLocales;
...
int __stdcall SFileOpenFileEx_lib(HANDLE hMPQ,  char * szFileName, uint dwSearchScope, HANDLE * phFile)
{
    ...
    uint dwHashIndex  = 0;             // Hash table index
    uint dwBlockIndex = (uint)-1;     // Found table index
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        if(hf->pBlock->dwFlags & MPQ_FILE_COMPRESSED)
        {
            ...
            // Allocate the buffer to include this uint as well
            
            if((hf->pdwBlockPos = ALLOCMEM(uint, hf->nBlocks + 2)) == NULL)
                nError = ERROR_NOT_ENOUGH_MEMORY;
        }
