     ...
     char * szNewFileName)
{
    ...
    TMPQBlock * pBlock = ha->pBlockTable + dwSaveBlockIndex;
     char * szPlainName;
    uint* pdwBlockPos1 = NULL;
    uint* pdwBlockPos2 = NULL;
    ...
    if(pBlock->dwFlags & MPQ_FILE_SINGLE_UNIT)
    {
        ...
        ReadFile(ha->hFile, pbFileBlock, pBlock->dwCSize, (LPDWORD)&dwTransferred, NULL);
        ...
        {
            // Recrypt the block
            DecryptMPQBlock((uint *)pbFileBlock, pBlock->dwCSize, dwOldSeed);
            EncryptMPQBlock((uint *)pbFileBlock, pBlock->dwCSize, dwNewSeed);

            // Write it back
            SetFilePointer(ha->hFile, MpqFilePos.LowPart, &MpqFilePos.HighPart, FILE_BEGIN);
            WriteFile(ha->hFile, pbFileBlock, pBlock->dwCSize, (LPDWORD)&dwTransferred, NULL);
            if(dwTransferred != pBlock->dwCSize)
    ...
    if(pBlock->dwFlags & MPQ_FILE_COMPRESSED)
    {
        ...
        pdwBlockPos1 = ALLOCMEM(uint, nBlocks + 2);
        pdwBlockPos2 = ALLOCMEM(uint, nBlocks + 2);
        if(pdwBlockPos1 == NULL || pdwBlockPos2 == NULL)
            return ERROR_NOT_ENOUGH_MEMORY;

        // Calculate number of bytes to be read
        dwToRead = (nBlocks + 1) * sizeof(uint);
        if(pBlock->dwFlags & MPQ_FILE_HAS_EXTRA)
            dwToRead += sizeof(uint);

        // Read the block positions
        SetFilePointer(ha->hFile, MpqFilePos.LowPart, &MpqFilePos.HighPart, FILE_BEGIN);
        ReadFile(ha->hFile, pdwBlockPos1, dwToRead, (LPDWORD)&dwTransferred, NULL);
        ...
        if(nError == ERROR_SUCCESS)
        {
            BSWAP_ARRAY32_UNSIGNED(pdwBlockPos1, dwToRead / sizeof(uint));
            ...
            BSWAP_ARRAY32_UNSIGNED(pdwBlockPos2, dwToRead / sizeof(uint));
        }
        ...
        {
            SetFilePointer(ha->hFile, MpqFilePos.LowPart, &MpqFilePos.HighPart, FILE_BEGIN);
            WriteFile(ha->hFile, pdwBlockPos2, dwToRead, (LPDWORD)&dwTransferred, NULL);
            if(dwTransferred != dwToRead)
                nError = ERROR_WRITE_FAULT;
        }
    }
    ...
    if(nError == ERROR_SUCCESS)
    {
        for(int nBlock = 0; nBlock < nBlocks; nBlock++)
        {
            ...
            ReadFile(ha->hFile, pbFileBlock, dwToRead, (LPDWORD)&dwTransferred, NULL);
            if(dwTransferred != dwToRead)
                nError = ERROR_FILE_CORRUPT;

            // Recrypt the file block
            BSWAP_ARRAY32_UNSIGNED((uint *)pbFileBlock, dwToRead/sizeof(uint));
            DecryptMPQBlock((uint *)pbFileBlock, dwToRead, dwOldSeed + nBlock);
            EncryptMPQBlock((uint *)pbFileBlock, dwToRead, dwNewSeed + nBlock);
            BSWAP_ARRAY32_UNSIGNED((uint *)pbFileBlock, dwToRead/sizeof(uint));

            // Write the block back
            SetFilePointer(ha->hFile, BlockFilePos.LowPart, &BlockFilePos.HighPart, FILE_BEGIN);
            WriteFile(ha->hFile, pbFileBlock, dwToRead, (LPDWORD)&dwTransferred, NULL);
            if(dwTransferred != dwToRead)
                nError = ERROR_WRITE_FAULT;
        }
    }
...
int __stdcall SFileCreateArchiveEx( char * szMpqName, uint dwCreationDisposition, uint dwHashTableSize, HANDLE * phMPQ)
{
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        uint dwHeaderSize = (wFormatVersion == MPQ_FORMAT_VERSION_2) ? sizeof(TMPQHeader2) : sizeof(TMPQHeader);
    ...
    if(nError == ERROR_SUCCESS)
    {
        uint dwHeaderSize = ha->pHeader->dwHeaderSize;

        BSWAP_TMPQHEADER(ha->pHeader);
        WriteFile(ha->hFile, ha->pHeader, dwHeaderSize, (LPDWORD)&dwTransferred, NULL);
        BSWAP_TMPQHEADER(ha->pHeader);
...
int __stdcall SFileAddFileEx(HANDLE hMPQ,  char * szFileName,  char * szArchivedName, uint dwFlags, uint dwQuality, int nFileType)
{
    TMPQArchive * ha = (TMPQArchive *)hMPQ;
    HANDLE hFile = INVALID_HANDLE_VALUE;
    int   bReplaced = FALSE;          // TRUE if replacing file in the archive
    int    nError = ERROR_SUCCESS;
...
//-----------------------------------------------------------------------------
// int SFileRemoveFile(HANDLE hMPQ, char * szFileName)
//
...
int __stdcall SFileRemoveFile(HANDLE hMPQ,  char * szFileName, uint dwSearchScope)
{
    TMPQArchive * ha = (TMPQArchive *)hMPQ;
