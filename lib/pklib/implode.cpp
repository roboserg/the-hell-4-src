...
static void SortBuffer(TCmpStruct * pWork, unsigned char * uncmp_data, unsigned char * work_end)
{
    ...
    ndwords = (unsigned long)((pWork->out_buff - (char *)pWork->phash_to_index + 1) >> 2);
    if(ndwords <= 1)
        ndwords = 1;
    memset(pWork->phash_to_index, 0, ndwords << 2);

    for(puncmp = uncmp_data; work_end > puncmp; puncmp++)
        pWork->phash_to_index[(puncmp[0] * 4) + (puncmp[1] * 5)]++;

    add = 0;
    for(pin0DC8 = pWork->phash_to_index; pin0DC8 < &pWork->phash_to_index_end; pin0DC8++)
    {
        add     += *pin0DC8;
    ...
    for(work_end--; work_end >= uncmp_data; work_end--)
    {
        ...
        pWork->phash_to_index[offs1]--;
        pWork->phash_offs[pWork->phash_to_index[offs1]] = (unsigned short)offs2;
    }
}
...
// to pWork structure.
static unsigned long FindRep(TCmpStruct * pWork, unsigned char * input_data)
{
    ...
    unsigned int *dsize)
{
    ...
    // it may fail.
    memset(work_buf, 0, sizeof(TCmpStruct));

    // Fill the work buffer information
    work_buf->read_buf    = read_buf;
    work_buf->write_buf   = write_buf;
    work_buf->dsize_bytes = *dsize;
    work_buf->ctype       = *type;
    work_buf->param       = param;
    work_buf->dsize_bits  = 4;
    work_buf->dsize_mask  = 0x0F;
    ...
    switch(*dsize)
    {
        case 0x1000 :
            work_buf->dsize_bits++;
            work_buf->dsize_mask |= 0x20;
            // No break here !!!

        case 0x0800 :
            work_buf->dsize_bits++;
            work_buf->dsize_mask |= 0x10;
            // No break here !!!
    ...
    switch(*type)
    {
        case CMP_BINARY: // We will compress data with binary compression type
            ...
            {
                work_buf->nChBits[nCount]  = 9;
                work_buf->nChCodes[nCount] = (unsigned short)nChCode;
        ...
        case CMP_ASCII: // We will compress data with ASCII compression type
            ...
            {
                work_buf->nChBits[nCount]  = (unsigned char )(ChBitsAsc[nCount] + 1);
                work_buf->nChCodes[nCount] = (unsigned short)(ChCodeAsc[nCount] * 2);
            }
            break;
    ...
    for(i = 0; i < 0x10; i++)
    {
        ...
        do
        {
            work_buf->nChBits[nCount]  = (unsigned char)(ExLenBits[i] + LenBits[i] + 1);
            work_buf->nChCodes[nCount] = (unsigned short)((nCount2 << (LenBits[i] + 1)) | ((LenCode[i] & 0xFFFF00FF) * 2) | 1);
    ...
    lmemcpy(&work_buf->dist_codes, DistCode, sizeof(DistCode));
    lmemcpy(&work_buf->dist_bits, DistBits, sizeof(DistBits));
    WriteCmpData(work_buf);
    return CMP_NO_ERROR;
}
