...
static void lmemcpy(void * trg,  void * src, size_t count)
{
...
static unsigned long Expand_pklib(TDcmpStruct * pWork)
{
        ...
        void         *param)
{
    ...
    memset(work_buf, 0, sizeof(TDcmpStruct));

    // Initialize work struct and load compressed data
    work_buf->read_buf   = read_buf;
    work_buf->write_buf  = write_buf;
    work_buf->param      = param;
    work_buf->in_pos     = sizeof(work_buf->in_buff);
    work_buf->in_bytes   = work_buf->read_buf((char *)work_buf->in_buff, &work_buf->in_pos, work_buf->param);
    if(work_buf->in_bytes <= 4)
        return CMP_BAD_DATA;

    work_buf->ctype      = work_buf->in_buff[0]; // Get the compression type
    work_buf->dsize_bits = work_buf->in_buff[1]; // Get the dictionary size
    work_buf->bit_buff   = work_buf->in_buff[2]; // Initialize 16-bit bit buffer
    work_buf->extra_bits = 0;                 // Extra (over 8) bits
    work_buf->in_pos     = 3;                 // Position in input buffer

    // Test for the valid dictionary size
    if(4 > work_buf->dsize_bits || work_buf->dsize_bits > 6) 
        return CMP_INVALID_DICTSIZE;

    work_buf->dsize_mask = 0xFFFF >> (0x10 - work_buf->dsize_bits); // Shifted by 'sar' instruction

    if(work_buf->ctype != CMP_BINARY)
    {
        if(work_buf->ctype != CMP_ASCII)
            return CMP_INVALID_MODE;

        lmemcpy(work_buf->ChBitsAsc, ChBitsAsc, sizeof(work_buf->ChBitsAsc));
        GenAscTabs(work_buf);
    }

    lmemcpy(work_buf->LenBits, LenBits, sizeof(work_buf->LenBits));
    GenDecodeTabs(0x10, work_buf->LenBits, LenCode, work_buf->position2);
    lmemcpy(work_buf->ExLenBits, ExLenBits, sizeof(work_buf->ExLenBits));
    lmemcpy(work_buf->LenBase, LenBase, sizeof(work_buf->LenBase));
    lmemcpy(work_buf->DistBits, DistBits, sizeof(work_buf->DistBits));
    GenDecodeTabs(0x40, work_buf->DistBits, DistCode, work_buf->position1);
    if(Expand_pklib(work_buf) != 0x306)
        return CMP_NO_ERROR;
