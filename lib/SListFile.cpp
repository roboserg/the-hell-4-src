...
struct TListFileCache
{
    ...
    uint   dwFileSize;                 // Total size of the cached file
    uint   dwBuffSize;                 // File of the cache
    uint   dwFilePos;                  // Position of the cache in the file
...
static int ReloadCache(TListFileCache * pCache)
{
    ...
    if(pCache->pPos >= pCache->pEnd)
    {
        ...
        SFileReadFile_lib(pCache->hFile, pCache->Buffer, pCache->dwBuffSize, &pCache->dwBuffSize, NULL);
...
{
    uint dwItems = ha->pHeader->dwHashTableSize;
...
int SListFileCreateNodeForAllLocales(TMPQArchive * ha,  char * szFileName)
{
    ...
    while(pHash->dwBlockIndex != HASH_ENTRY_FREE)
    {
        ...
        if(pHash->dwBlockIndex != HASH_ENTRY_DELETED)
        {
            if(pHash->dwName1 == dwName1 && pHash->dwName2 == dwName2)
            {
                ...
                dwHashIndex = (uint)(pHash - ha->pHashTable);
...
int SListFileCreateNode(TMPQArchive * ha,  char * szFileName, LCID lcLocale)
{
    ...
    dwHashIndex0 = (uint)(pHash0 - ha->pHashTable);
    dwHashIndex1 = (uint)(pHash1 - ha->pHashTable);
...
void SListFileFreeListFile(TMPQArchive * ha)
{
    if(ha->pListFile != NULL)
    {
        for(uint i = 0; i < ha->pHeader->dwHashTableSize; i++)
        {
...
int SListFileSaveToMpq(TMPQArchive * ha)
{
    ...
    if(nError == ERROR_SUCCESS)
    {
        for(;;)
        {
            if(pHash->dwName1 != dwName1 && pHash->dwName2 != dwName2 && pHash->dwBlockIndex < HASH_ENTRY_DELETED)
            {
                ...
                if((DWORD_PTR)pNode < LISTFILE_ENTRY_DELETED)
                {
                    ...
                    WriteFile(hFile, szBuffer, (uint)(pNode->nLength + 2), (LPDWORD)&dwTransferred, NULL);
                }
            }
        ...
        if(GetHashEntry(ha, LISTFILE_NAME) == NULL)
        {
            ...
            WriteFile(hFile, szBuffer, (uint)(nLength + 2), (LPDWORD)&dwTransferred, NULL);
        }                     
        
        // Add the listfile into the archive.
        SFileSetLocale_lib(LANG_NEUTRAL);
...
int __stdcall SFileAddListFile(HANDLE hMpq,  char * szListFile)
{
    ...
    uint dwSearchScope = SFILE_OPEN_LOCAL_FILE;
    uint dwCacheSize = 0;
    uint dwFileSize = 0;
    size_t nLength = 0;
    int nError = ERROR_SUCCESS;
    ...
    if(nError == ERROR_SUCCESS)
    {
        if(!SFileOpenFileEx_lib((HANDLE)ha, szListFile, dwSearchScope, &hListFile))
            nError = GetLastError();
    }
    ...
    {
        dwCacheSize = 
        dwFileSize = SFileGetFileSize_lib(hListFile, NULL);
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        // Fill the cache
        SFileReadFile_lib(hListFile, pCache->Buffer, pCache->dwBuffSize, &pCache->dwBuffSize, NULL);
...
int SListFileFindClose(HANDLE hFind)
{
    ...
    if(pCache != NULL)
    {
        if(pCache->hFile != NULL)
            SFileCloseFile_lib(pCache->hFile);
