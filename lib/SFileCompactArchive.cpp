...
static int CheckIfAllFilesKnown(TMPQArchive * ha,  char * szListFile, uint * pFileSeeds)
{
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        for(pHash = pHashTableCopy; pHash < pHashEnd; pHash++)
        {
            ...
            if(pHash->dwBlockIndex < ha->pHeader->dwBlockTableSize)
            {
                ...
                if(SFileOpenFileEx_lib((HANDLE)ha, (char *)(DWORD_PTR)pHash->dwBlockIndex, SFILE_OPEN_BY_INDEX, &hFile))
                {
                    TMPQFile * hf = (TMPQFile *)hFile;
                    dwFlags = hf->pBlock->dwFlags;
                    dwSeed = hf->dwSeed1;
                    SFileCloseFile_lib(hFile);
                }
    ...
    uint dwSeed)
{
    ...
    uint * pdwBlockPos2 = NULL;        // File block positions to be written to target file
    uint * pdwBlockPos = NULL;         // File block positions (unencrypted)
    BYTE  * pbBlock = NULL;             // Buffer for the file block
    uint dwTransferred;                // Number of bytes transferred
    uint dwCSize = 0;                  // Compressed file size
    ...
    if(nError == ERROR_SUCCESS && (pBlock->dwFlags & MPQ_FILE_ENCRYPTED))
    {
        ...
        dwSeed2 = dwSeed;
        if(pBlock->dwFlags & MPQ_FILE_FIXSEED)
            dwSeed2 = (dwSeed + (uint)(FilePos.QuadPart - ha->MpqPos.QuadPart)) ^ pBlock->dwFSize;
    }
    ...
    if(pBlock->dwFlags & MPQ_FILE_COMPRESSED)
    {
        ...
        if(nError == ERROR_SUCCESS)
        {
            pdwBlockPos = ALLOCMEM(uint, nBlocks + 2);
        ...
        if(nError == ERROR_SUCCESS)
        {
            dwBytes = (nBlocks + 1) * sizeof(uint);
            if(pBlock->dwFlags & MPQ_FILE_HAS_EXTRA)
                dwBytes += sizeof(uint);

            ReadFile(ha->hFile, pdwBlockPos, dwBytes, (LPDWORD)&dwTransferred, NULL);
        ...
        if(nError == ERROR_SUCCESS)
        {
            BSWAP_ARRAY32_UNSIGNED(pdwBlockPos, dwBytes / sizeof(uint));
            if(pBlock->dwFlags & MPQ_FILE_ENCRYPTED)
            {
            ...
            BSWAP_ARRAY32_UNSIGNED(pdwBlockPos2, dwBytes / sizeof(uint));
        }

        // Write to the target file
        if(nError == ERROR_SUCCESS)
        {
            WriteFile(hFile, pdwBlockPos2, dwBytes, (LPDWORD)&dwTransferred, NULL);
    ...
    if(nError == ERROR_SUCCESS)
    {
        for(nBlock = 0; nBlock < nBlocks; nBlock++)
        {
            ...
            // Read the file block
            ReadFile(ha->hFile, pbBlock, dwBytes, (LPDWORD)&dwTransferred, NULL);
            if(dwTransferred != dwBytes)
            {
                nError = ERROR_FILE_CORRUPT;
                break;
            ...
            {
                BSWAP_ARRAY32_UNSIGNED((uint *)pbBlock, dwBytes/sizeof(uint));
            ...
            WriteFile(hFile, pbBlock, dwBytes, (LPDWORD)&dwTransferred, NULL);
            dwCSize += dwTransferred;
            if(dwTransferred != dwBytes)
            {
                nError = ERROR_DISK_FULL;
    ...
    LARGE_INTEGER & DataSizeToCopy)
{
    ...
    while(DataSize.QuadPart > 0)
    {
        ...
        ReadFile(hSrcFile, DataBuffer, dwToRead, (LPDWORD)&dwTransferred, NULL);
        ...
        // Write to the target file
        WriteFile(hTrgFile, DataBuffer, dwToRead, (LPDWORD)&dwTransferred, NULL);
        if(dwTransferred != dwToRead)
        {
            nError = ERROR_DISK_FULL;
            break;
...
int __stdcall SFileSetCompactCallback(HANDLE /* hMPQ */, COMPACTCB aCompactCB, void * lpData)
{
...
int __stdcall SFileCompactArchive(HANDLE hMPQ,  char * szListFile, int /* bReserved */)
{
    ...
    if(nError == ERROR_SUCCESS && ha->pShunt != NULL)
    {
        ...
        WriteFile(hFile, ha->pShunt, sizeof(TMPQShunt), (LPDWORD)&dwTransferred, NULL);
        BSWAP_TMPQSHUNT(ha->pShunt);
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        WriteFile(hFile, ha->pHeader, ha->pHeader->dwHeaderSize, (LPDWORD)&dwTransferred, NULL);
        BSWAP_TMPQHEADER(ha->pHeader);
